/**
 * Build a fast-sync reference bundle for a given network.
 *
 * This creates a throwaway wallet, syncs it fully (including DUST),
 * then serializes all three sub-wallet states into gzipped files.
 *
 * Usage: npx tsx scripts/build-reference-bundle.ts <networkId> <outputDir>
 * Example: npx tsx scripts/build-reference-bundle.ts preprod ./preseed
 */
...
import { ZswapSecretKeys, DustSecretKey, LedgerParameters } from '@midnight-ntwrk/wallet-sdk/ledger/v9';
import { setNetworkId } from '@midnight-ntwrk/midnight-js-network-id';
import * as Rx from 'rxjs';

const NETWORK_ID = process.argv[2] || 'preprod';
const OUTPUT_DIR = process.argv[3] || './preseed';

const ENDPOINTS: Record<string, { indexer: string; indexerWS: string; nodeWS: string; proofServer: string }> = {
  preprod: {
...
async function main() {
  const endpoints = ENDPOINTS[NETWORK_ID];
  if (!endpoints) {
    console.error(`Unknown network: ${NETWORK_ID}. Supported: ${Object.keys(ENDPOINTS).join(', ')}`);
    process.exit(1);
  }

  console.log(`Building reference bundle for ${NETWORK_ID}...`);
  setNetworkId(NETWORK_ID);

  // Step 1: Generate a throwaway mnemonic
  const mnemonic = Array.from({ length: 24 }, () => {
    const words = ['abandon', 'ability', 'able', 'about', 'above', 'absent', 'absorb', 'abstract', 'absurd', 'abuse',
      ...
      'antenna', 'antique', 'anxiety', 'any', 'apart', 'apology', 'appear', 'apple', 'approve', 'april', 'arch',
      'arctic', 'area', 'arena', 'argue', 'arm', 'armed', 'armor', 'army', 'around', 'arrange', 'arrest', 'arrive',
      ...
      'author', 'auto', 'autumn', 'average', 'avocado', 'avoid', 'awake', 'aware', 'awesome', 'awful', 'awkward',
      ...
      'black', 'blade', 'blame', 'blanket', 'blast', 'bleak', 'bless', 'blind', 'blood', 'blossom', 'blow', 'blue',
      ...
      'bunny', 'burden', 'burger', 'burst', 'bus', 'business', 'busy', 'butter', 'buyer', 'buzz', 'cabbage', 'cabin',
      ...
      'chalk', 'champion', 'change', 'chaos', 'chapter', 'charge', 'chase', 'cheap', 'check', 'cheese', 'chef', 'cherry',
      'chest', 'chicken', 'chief', 'child', 'chimney', 'choice', 'choose', 'chronic', 'chuckle', 'chunk', 'churn',
      'citizen', 'city', 'civil', 'claim', 'clap', 'clarify', 'claw', 'clay', 'clean', 'clerk', 'clever', 'cliff',
      ...
      'crop', 'cross', 'crouch', 'crowd', 'crucial', 'cruel', 'cruise', 'crumble', 'crush', 'cry', 'crystal', 'cube',
      ...
      'embark', 'embody', 'embrace', 'emerge', 'emotion', 'employ', 'empower', 'empty', 'enable', 'encourage', 'end',
      ...
      'filter', 'final', 'find', 'fine', 'finger', 'finish', 'fire', 'firm', 'fiscal', 'fish', 'fit', 'fitness',
      ...
      'industry', 'infant', 'inflict', 'inform', 'initial', 'inject', 'inmate', 'inner', 'innocent', 'input', 'inquiry',
      ...
      'loop', 'lottery', 'loud', 'lounge', 'love', 'loyal', 'lucky', 'luggage', 'lumber', 'lunar', 'lunch', 'luxury',
      ...
      'marriage', 'mask', 'mass', 'master', 'match', 'material', 'math', 'matrix', 'matter', 'maximum', 'maze', 'meadow',
      'mean', 'measure', 'meat', 'mechanic', 'medal', 'media', 'melody', 'melt', 'member', 'memory', 'mention', 'menu',
      'mercy', 'merge', 'merit', 'merry', 'mesh', 'message', 'metal', 'method', 'middle', 'midnight', 'milk', 'million',
      'mimic', 'mind', 'minimum', 'minor', 'minute', 'miracle', 'mirror', 'misery', 'miss', 'mistake', 'mix', 'mixed',
      'mixture', 'mobile', 'model', 'modify', 'mom', 'moment', 'monitor', 'monkey', 'monster', 'month', 'moon', 'moral',
      'more', 'morning', 'mosquito', 'mother', 'motion', 'motor', 'mountain', 'mouse', 'move', 'movie', 'much', 'muffin',
      ...
      'noble', 'noise', 'nominee', 'noodle', 'normal', 'north', 'nose', 'notable', 'nothing', 'notice', 'novel',
      ...
      'polar', 'pole', 'police', 'pond', 'pony', 'pool', 'popular', 'portion', 'position', 'possible', 'post', 'potato',
      ...
      'quiz', 'quote', 'rabbit', 'raccoon', 'race', 'rack', 'radar', 'radio', 'rage', 'rail', 'rain', 'raise',
      ...
      'reflect', 'reform', 'region', 'regret', 'regular', 'reject', 'relax', 'release', 'relief', 'rely', 'remain',
      ...
      'right', 'rigid', 'ring', 'riot', 'ripple', 'risk', 'ritual', 'rival', 'river', 'road', 'roast', 'robot',
      'robust', 'rocket', 'romance', 'roof', 'rookie', 'room', 'rose', 'rotate', 'rough', 'round', 'route', 'royal',
      'rubber', 'rude', 'rug', 'rule', 'run', 'runway', 'rural', 'sad', 'saddle', 'sadness', 'safe', 'sail',
      ...
      'swarm', 'swear', 'sweet', 'swim', 'swing', 'switch', 'sword', 'symbol', 'symptom', 'syrup', 'system', 'table',
      ...
      'tube', 'tuna', 'tunnel', 'turkey', 'turn', 'turtle', 'twelve', 'twenty', 'twice', 'twin', 'twist', 'two',
      ...
      'update', 'upgrade', 'uphold', 'upon', 'upper', 'upset', 'urban', 'usage', 'use', 'used', 'useful', 'useless',
      ...
      'very', 'vessel', 'veteran', 'viable', 'vibrant', 'vicious', 'victory', 'video', 'view', 'village', 'vintage',
      'violin', 'virtual', 'virus', 'visa', 'visit', 'visual', 'vital', 'vivid', 'vocal', 'voice', 'void', 'volcano',
      'volume', 'vote', 'voyage', 'wage', 'wagon', 'wait', 'walk', 'wall', 'walnut', 'want', 'warfare', 'warm',
      'warrior', 'wash', 'wasp', 'waste', 'water', 'wave', 'way', 'wealth', 'weapon', 'wear', 'weasel', 'weather',
      'web', 'wedding', 'weekend', 'weird', 'welcome', 'well', 'west', 'wet', 'whale', 'what', 'wheat', 'wheel',
      'when', 'where', 'whip', 'whisper', 'wide', 'width', 'wife', 'wild', 'will', 'win', 'window', 'wine',
  ...
  console.log('Generated throwaway mnemonic (24 words)');

  // Step 2: Derive keys
  const { HDWallet: HDWalletClass } = await import('@midnight-ntwrk/wallet-sdk/hd');
  const hdResult = HDWalletClass.fromMnemonic(mnemonic);
  if (hdResult.type !== 'mnemonicOk') {
    throw new Error(`Failed to derive HD wallet: ${hdResult.type}`);
  }
  ...
  const keystore = createKeystore(derivationResult.keys[Roles.NightExternal], NETWORK_ID);
  const unshieldedPublicKey = PublicKey.fromKeyStore(keystore);
  hdWallet.clear();

  console.log('Keys derived');

  // Step 3: Build wallet facade
  const config = {
  ...
  const facade = await WalletFacade.init({
    ...
    shielded: (cfg) => CustomShieldedWallet(cfg).startWithSecretKeys(shieldedSecretKeys as any),
    unshielded: (cfg) => UnshieldedWallet(cfg).startWithPublicKey(unshieldedPublicKey),
    dust: (cfg) => CustomDustWallet(cfg).startWithSecretKey(
  ...
  await facade.start(shieldedSecretKeys as any, dustSecretKey as any);
  console.log('Wallet started, syncing...');

  // Wait for full sync (including dust)
  const state = await Rx.firstValueFrom(
    facade.state().pipe(
      Rx.filter((s: any) => {
        ...
        return (
          sh?.isStrictlyComplete?.() &&
          un?.isStrictlyComplete?.() &&
          dust?.isStrictlyComplete?.()
        );
      }),
      Rx.take(1),
    ),
  ...
  const chainHeight = dustProgress?.highestRelevantWalletIndex?.toString() ?? 'unknown';
  console.log(`Sync complete. Chain height: ${chainHeight}`);

  // Step 5: Serialize sub-wallet states
  const shieldedState = (state as any).shielded?.serialize?.();
  const unshieldedState = (state as any).unshielded?.serialize?.();
  const dustState = (state as any).dust?.serialize?.();

  if (!shieldedState || !unshieldedState || !dustState) {
    console.error('Failed to serialize wallet states');
    process.exit(1);
  }

  // Step 6: Save as gzipped files
  const outDir = join(OUTPUT_DIR, NETWORK_ID);
  mkdirSync(outDir, { recursive: true });

  const manifest = {
    network: NETWORK_ID,
    height: Number(chainHeight),
    parts: {} as Record<string, { bytes: number; gzipBytes: number }>,
  };

  for (const [name, data] of [['shielded', shieldedState], ['unshielded', unshieldedState], ['dust', dustState]] as const) {
    const raw = Buffer.from(data, 'utf8');
    const gzipped = gzipSync(raw);
    const filePath = join(outDir, `${name}.dat.gz`);
    writeFileSync(filePath, gzipped);
    manifest.parts[name] = { bytes: raw.length, gzipBytes: gzipped.length };
    console.log(`${name}: ${raw.length} bytes → ${gzipped.length} bytes (${filePath})`);
  }

  const manifestPath = join(outDir, 'manifest.json');
  writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));
  console.log(`Manifest: ${manifestPath}`);
  console.log(`Reference bundle built for ${NETWORK_ID} at height ${chainHeight}`);
