...
export function DAppWalletPage() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { stage, request, startWalletConnect } = useDAppVerification()
  ...
  useEffect(() => {
    ...
    if (stage === 'wallet-connected') {
      navigate(`/dapp/${id}/disclosure`)
    }
  ...
  return (
    <div style={{
      ...
      >
        <button
          onClick={() => navigate(`/dapp/verify?req=${request.id}`)}
          style={{
        ...
        <div style={{ marginBottom: 'var(--space-8)' }}>
          ...
          <p style={{
            ...
            Connect a Midnight wallet to verify for <strong style={{ color: 'var(--text-primary)' }}>{request.appName}</strong>. Your wallet is a handle, not an identity.
          </p>
        </div>
        ...
        <div style={{
          ...
          The wallet signs the scoped commitment used to complete this verification. It is not used as an identity.
        </div>
